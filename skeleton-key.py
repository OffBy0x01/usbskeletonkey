...
class SkeletonKey:
    ...
    def discover_modules(self, dir_path):
    ...
    def init_config(self):
        if not (os.path.exists(self.dir_path+"/modules")):
            # doesn't exist - user can fix themselves
            print("Error: "+self.dir_path+"\"modules\" directory does not exist!")
            exit()

        present_modules = self.get_modules()
        ...
        config.read(self.dir_path+'/config.ini')
