...
class FwComponentNetwork(FwComponentGadget):
    ...
    def up(self):
        ...
        self.network.debug(Format.color_danger + "Failed to ifup usb0" + Format.format_clear
        ...
        self.network.debug(Format.color_danger + "Failed to up networking on usb0" + Format.format_clear
                           if usb0_ifconfig else "usb0 networking up")  # Up networking on usb0
        ...
        self.network.debug(Format.color_danger + "Failed to add IP routes to usb0" + Format.format_clear
                           if usb0_routes else "usb0 IP routes added successfully")  # Add route for all IPv4 addresses

        if usb0_routes:  # If process failed return False
            return False

        dhcp = subprocess.call("/etc/init.d/isc-dhcp-server start", shell=True)
        self.network.debug(Format.color_danger + "Failed to start DHCP server" + Format.format_clear
                           if dhcp else "DHCP server successfully started")  # Start DHCP server

        if dhcp:  # If process failed return False
            return False

        ip_forwarding = subprocess.call("/sbin/sysctl -w net.ipv4.ip_forward=1", shell=True)
        self.network.debug(Format.color_danger + "Failed to enable IPv4 forwarding" + Format.format_clear
                           if ip_forwarding else "IPv4 forwarding successfully enabled")  # Enable IPv4 forwarding

        if ip_forwarding:  # If process failed return False
            return False

        bind = subprocess.call("/sbin/iptables -t nat -A PREROUTING -i usb0 -p tcp --dport 80 -j REDIRECT "
                               "--to-port 1337", shell=True)
        self.network.debug(Format.color_danger + "Failed to bind port 80 to 1337" + Format.format_clear
                           if bind else "Successfully binded port 80 to port 1337")  # Bind port 80 to port 1337
        ...
        self.network.debug(Format.color_danger + "Failed to start dnsspoof on port 53" + Format.format_clear
                           if dnsspoof else "Successfully started dnsspoof on port 53")  # Start dnsspoof on port 53
    ...
    def down(self):
        ...
        self.network.debug(Format.color_danger + "Failed to disable IPv4 forwarding" + Format.format_clear
                           if subprocess.call("/sbin/sysctl -w net.ipv4.ip_forward=0", shell=True) else
                           "IPv4 forwarding successfully disabled")  # Disable IPv4 forwarding

        self.network.debug(Format.color_danger + "Failed to stop DHCP server" + Format.format_clear
                           if subprocess.call ("/etc/init.d/isc-dhcp-server stop", shell=True)
                           else "DHCP server successfully stopped")  # Stop DHCP server

        self.network.debug(Format.color_danger + "Failed to remove IP routes from usb0" + Format.format_clear
                           if subprocess.call("/sbin/route del -net 0.0.0.0/0 usb0", shell=True)
                           else "usb0 IP routes removed successfully")  # Remove route for all IPv4 addresses

        # Down adapter
        self.network.debug(Format.color_danger + "Failed to down networking on usb0" + Format.format_clear if
                           subprocess.call("ifconfig usb0 down", shell=True)
                           else "usb0 networking down")  # Down networking on usb0

        self.network.debug(Format.color_danger + "Failed to ifdown usb0" + Format.format_clear
                           if subprocess.call("ifdown usb0", shell=True)
